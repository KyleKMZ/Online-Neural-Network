		...
		if entries:
			for entry in entries:
				...
				for sub_entry in sub_entries:
					...
					entry_node.append(sub_entry_node)

				self.parentItem.append(entry_node)
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.update_dbModel()
        self.dbTreeView = QTreeView()
        self.dbTreeView.setModel(self.dbModel)
    ...
    def update_dbModel(self):
    	...
    	for entry in entries:
    		...
    		subEntries = [os.path.join(entry, f) for f in os.listdir(entry) if os.path.isdir(os.path.join(entry, f))]
    		for i in range(0, len(subEntries)):
    			...
    			with open(os.path.join(subEntries[i], 'info.txt'), 'r') as info_f:
    				...
    				amp_cont = info_f.readlin().split()[-1]
    				psize = info_f.readline().split()[-1]

    				entry_List[i+1]['job_type'] = os.path.split(subEntries[i])[-1]
    				entry_List[i+1]['num_particles'] = num_particles
    				entry_list[i+1]['num_mics'] = num_mics
    				entry_list[i+1]['voltage'] = voltage
    				entry_list[i+1]['cs'] = cs
    				entry_list[i+1]['amp_cont'] = amp_cont
    				entry_list[i+1]['psize'] = psize
    				entry_list[i+1]['job_size'] = self.get_size_format(self.get_directory_size(entry))
    		self.dbModel.entries.append(entryList)

    	self.dbModel.layoutChanged.emit()

    def update_metada_info(self):
    	pass
